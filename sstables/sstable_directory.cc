...
sstable_directory::handle_component(scan_state& state, sstables::entry_descriptor desc, fs::path filename) {
    if (calc_shard_num(desc.generation) != this_shard_id()) {
        return;
    }
...
sstable_directory::process_sstable_dir(const ::io_priority_class& iop, bool sort_sstables_according_to_owner) {
    ...
    _max_generation_seen =  boost::accumulate(state.generations_found | boost::adaptors::map_keys, generation_type{}, [] (generation_type a, generation_type b) {
        return std::max<generation_type>(a, b);
    });

    dirlog.debug("After {} scanned, seen generation {}. {} descriptors found, {} different files found ",
            _sstable_dir, to_string(_max_generation_seen), state.descriptors.size(), state.generations_found.size());
