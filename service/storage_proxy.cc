...
future<> storage_proxy::replicate_counter_from_leader(mutation m, db::consistency_level cl, tracing::trace_state_ptr tr_state,
    ...
    return f_cdc.then([this, cl, tr_state, timeout, permit = std::move(permit)]
        ...
        std::vector<mutation> base_mutations, cdc_mutations;
        std::for_each(std::make_move_iterator(mutations.begin()), std::make_move_iterator(mutations.end()),
                [&base_mutations, &cdc_mutations] (mutation&& m) {
            (m.schema()->is_counter() ? base_mutations : cdc_mutations).push_back(std::move(m));
        });
        
        // FIXME: do not send the mutation to itself, it has already been applied (it is not incorrect to do so, though)
        return seastar::when_all_succeed(
            mutate_internal(std::move(cdc_mutations), cl, false, tr_state, permit, timeout),
            mutate_internal(std::move(base_mutations), cl, true, tr_state, permit, timeout)
        );
    });
}
